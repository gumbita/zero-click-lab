...
)
{
    ...
    *result = (parser_result){0};
    ...
    if (payload == NULL) {
        result->status = PARSER_ALLOCATION_FAILED;
        return result->status;
    }

    /*
     * Vulnerabilidad deliberada del laboratorio: el destino admite 32 bytes,
     * pero la copia confía en la longitud declarada sin validar ese máximo.
     */
    (void)memcpy(
        payload,
        &data[PACKET_OFFSET_PAYLOAD],
        (size_t)result->declared_length
    );

    for (index = 0u; index < (size_t)result->declared_length; ++index) {
